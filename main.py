...
# 获取当前时间对应的最大和最小步数【夜间低步数，真人曲线，必改！】
...
class MiMotionRunner:
    ...
    def login_and_post_step(self, min_step, max_step):
        ...
        self.log_str += f"✅ 夜间真人模式 | 范围({min_step}~{max_step}) 步数:{step}\n"
    ...
    if len(user_list) == len(passwd_list):
        ...
        if use_concurrent:
            ...
            for user_mi, passwd_mi in zip(user_list, passwd_list):
                ...
                idx += 1
                if idx < total:
                    time.sleep(max(sleep_seconds, 8))
        if encrypt_support:
            persist_user_tokens()
        success_count = 0
        push_results = []
...
if __name__ == "__main__":
    ...
    if os.environ.__contains__("CONFIG") is False:
        ...
        use_concurrent = (use_concurrent is not None and use_concurrent == 'True')
        execute()
